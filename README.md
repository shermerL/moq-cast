...
本项目的主要功能为跨网屏幕共享。通过服务器自行部署 `moq-relay` 服务，可以解决复杂 NAT 环境下的连接问题。它是基于 [Media over QUIC](https://moq.dev/) 的原生 Android 应用，可以采集 Android 屏幕并编码为 H.264，发布到指定 broadcast，其他客户端可连接到 `moq-relay` 订阅并播放。当然，MoQ 并不限于屏幕共享，更多信息请访问 [Media over QUIC](https://moq.dev/)。
...
- 可发起屏幕共享
- 可发布 Android 系统音频（Android 10 及以上）
- 可订阅并播放视频流
...
- `RECORD_AUDIO`：在 Android 10 及以上版本采集系统播放音频

## 当前限制

- 屏幕发布仅支持 H.264 视频
- 系统音频发布需要 Android 10 及以上版本，且被采集应用必须允许系统音频捕获
- 暂不采集麦克风音频
- 暂无编码协商
- 尚未加入自动化测试和签名配置
